...
Protected Module Info
	#tag Note, Name = 1. Info
		...
		Known Issues: 
		• Linux has a problem positioning the caret, as I don't have a linux box, this issue is unlikely
	...
	#tag Note, Name = 2. Release Notes
		...
		• SelectionChanged/TextChange events are no longer fired multiple times when undoing
	...
	#tag Note, Name = 3. Docs
		********************************************************************************************************************************************** Global Settings:
		...
		TextStorageType: determines the kind of structure used internally by the editfield to store text,
		...
		UseOldRenderer:
		This Boolean value forces the field to draw its contents using QuickDraw instead of Quartz 
		(this setting is only meaningful under OSX).
		...
		********************************************************************************************************************************************** 
		...
		To obtain a CustomEditFieldPrinter object, simply call CustomEditField.CustomEditFieldPrinter(g as Graphics)
		 and pass the graphics context where you want to draw onto,
		...
		********************************************************************************************************************************************** 
		...
		Inserts the given text at the given index (offset). This method is used internally by the control, and externally by the undo mechanism, you shouldn't use it direclty, use instead cefSelStart and seltext.
		...
		This method is used internally by the control, and externally by the undo mechanism, you shouldn't use it direclty, use instead cefSelStart and seltext.
		...
		********************************************************************************************************************************************** 
		...
		CaretPos as Integer:
		Sets/gets the position of the caret. when cefSelLength = 0, caretPos and cefSelStart are the same.
		
		ClearHighlightedRangesOnTextChange as Boolean
		If true all highlighted character ranges will be cleared when the text is changed, and before the TextChange event is raised. You can re-calculate ranges in the TextChange event
		and add them to the field, these will be redrawn on the next update which should be after the TextChange event is handled.
